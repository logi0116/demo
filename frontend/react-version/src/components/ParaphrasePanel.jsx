...
export function ParaphrasePanel({ inputText, setInputText }) {
  ...
  const handleParaphrase = async () => {
    try {
      ...
      const result = await apiService.paraphraseText({
        text: validatedText,
      ...
      setTimeout(() => {
        ...
        toast({
          title: "의역 완료",
          description: `${paraphraseStyles.find((s) => s.value === paraphraseStyle)?.label} 스타일로 의역되었습니다.`,
        })
      }, 500)
    } catch (err) {
